<template>
  <q-page class="home">
    ...
    <!-- ✅ ZONA FIJA: carousel + search nunca se mueven -->
    ...
    <div v-if="modoFiesta && selectedEvent" class="fiesta-bar">
      ...
      <div class="fiesta-buttons">
        <q-btn class="fiesta-btn" icon="local_bar"           label="Bebidas"    unelevated @click="goCategory('bebidas')"    />
        <q-btn class="fiesta-btn" icon="emoji_food_beverage" label="Coctelería" unelevated @click="goCategory('cocteleria')" />
        <q-btn class="fiesta-btn" icon="liquor"              label="Destilados" unelevated @click="goCategory('destilados')" />
      </div>
    </div>

    <!-- EVENTOS — única zona que scrollea -->
        ...
        <div class="row no-wrap">
          ...
          <div class="q-pa-sm column justify-between col">
            <div>
              ...
              <div class="event-sub">{{ event.date }} · {{ event.location }}</div>
            </div>
            <q-btn
              dense unelevated
              class="buy-btn"
    ...
    <q-dialog v-model="barModal">
      <q-card style="width:350px;background:#020617;color:white">
        <q-card-section class="text-h6">🍾 {{ categorySelected }}</q-card-section>
        <q-separator />
        <q-card-section>
          <div v-if="barItems.length === 0" class="text-grey text-center">No hay productos</div>
          <q-list v-else>
            <q-item v-for="item in barItems" :key="item.id" clickable @click="buyDrink(item)">
              <q-item-section avatar>
                <img :src="item.image" style="width:40px;height:40px;border-radius:8px" />
              </q-item-section>
              <q-item-section>{{ item.name }}</q-item-section>
              <q-item-section side>S/ {{ item.price }}</q-item-section>
            </q-item>
          </q-list>
        </q-card-section>
      </q-card>
...
const loadEvents = async () => {
  ...
  try {
    let res
    if (modoFiesta.value) {
      res = await api.get('/auth/my-tickets', { headers: { Authorization: `Bearer ${token}` } })
      const events = res.data.map(ticket => ticket.event)
      featuredEvents.value = [...new Map(events.map(e => [e.id, e])).values()]
    } else {
      res = await api.get('/auth/events', { headers: { Authorization: `Bearer ${token}` } })
...
const buyDrink = async (item) => {
  try {
    ...
    await Swal.fire({
      ...
      html: `<p>Muestra este QR en la barra</p><img src="${order.qr}" style="width:200px"/>`,
...
const goCategory = (cat) => {
  if (!selectedEvent.value) { console.log('No hay evento seleccionado'); return }
  loadBarItems(cat)
}
...
onMounted(() => {
  ...
  if (savedModo !== null) modoFiesta.value = savedModo === 'true'
  loadEvents()
...
.home {
  background: #0b0f19;
  /* Altura real disponible tras el header de Quasar */
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ZONA FIJA — carousel + search
   Pegada arriba dentro del q-page, sin moverse
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
.fixed-top-zone {
  position: absolute;
  top: 0;
...
.fiesta-btn:hover { background: #1f2937; }

/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   EVENTOS — ÚNICA ZONA QUE SCROLLEA
   top = 210px carousel + ~60px search = 270px
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
.events-scroll {
  position: absolute;
  top: 270px;   /* altura carousel (210) + search (~60) */
  left: 0;
  right: 0;
  bottom: 0;
  overflow-y: auto;
...
.event-title { font-weight: 800; font-size: 0.95rem; }
.event-sub   { font-size: 0.75rem; color: #94a3b8; margin-bottom: 6px; }
.buy-btn     { background: #2563eb; color: white; font-weight: 700; border-radius: 10px; }
